...
class TripartiteGraph:
    ...
    def get_graph_from_table(self, df, table, idx_rows):
        ...
        # Create Row Nodes
        rids = list(map(lambda x: table + '_row_' + str(x), idx_rows))
        G.add_nodes_from(rids, type='row', filename=table)

        # Add edges between cell nodes and attributes 
        # Add edges between cell nodes and row
        for c, n_c in zip(df.columns, nodes_attributes):
            ...
            edges_rows = list(zip(rids, values))
